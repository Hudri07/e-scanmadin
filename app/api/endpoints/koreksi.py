...
from app.services.omr_service import scan_halaman_omr
...
async def proses_kunci(file: UploadFile = File(...), db: Session = Depends(get_db)):
    file_path = os.path.join(UPLOAD_DIR, "kunci_temp.jpg")
...
async def scan_bulk(
    ...
    for file in files:
        ...
        try:
            ...
            # --- PROSES SKOR ---
            jawaban_siswa = scan_halaman_omr(file_path)
            if jawaban_siswa:
            ...
            print(f"Error saat memproses file {file.filename}: {e}") # Tambahkan print untuk debugging
            db.rollback()
            continue
